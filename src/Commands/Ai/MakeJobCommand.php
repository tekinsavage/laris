...
class MakeJobCommand extends Command
{
    ...
    {
        $this
            ->setName('laris:ai:make:job')
            ->setDescription('Generate a Laravel Job class using OpenRouter AI');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $name = $io->ask('What is the name of the Job class?', 'SendEmailJob');

        $prompt = <<<PROMPT
{$defaultPrompt}

You are a Laravel 10 expert. Generate a Laravel Job class named "{$name}". Assume this job sends an email or performs some queued background task. Make it implement the ShouldQueue interface and use the Dispatchable, InteractsWithQueue, and Queueable traits. Follow PSR-12 and Laravel queueing conventions.
PROMPT;

        $io->section('🧠 Generating job using AI...');
        $io->text('Please wait...');
        sleep(3);
        ...
        $io->section('✅ AI Generated Job Code');
        $io->writeln("<info>$code</info>");

        $save = $io->confirm('Do you want to save this job to app/Jobs?', true);
        if ($save) {
            $path = getcwd() . "/app/Jobs/{$name}.php";
            file_put_contents($path, $code);
            $io->success("Job saved to app/Jobs/{$name}.php");
        }
