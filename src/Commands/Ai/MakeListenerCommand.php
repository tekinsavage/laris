...
class MakeListenerCommand extends Command
{
    ...
    {
        $this
            ->setName('laris:ai:make:listener')
            ->setDescription('Generate a Laravel listener using OpenRouter AI');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $listenerName = $io->ask('What is the name of the listener?', 'ExampleListener');

        $prompt = <<<PROMPT
{$defaultPrompt}

You are a Laravel assistant. Generate a Laravel Event Listener class named "{$listenerName}". Make sure it is clean, PSR-12 compliant, and ready to handle a custom event. Target Laravel 10.
PROMPT;

        $io->section('🧠 Generating listener code...');
        $io->write('Thinking');
        sleep(1); $io->write('.'); sleep(1); $io->write('.'); sleep(1); $io->writeln('.');
        ...
        $save = $io->confirm('Save this listener?', true);
        if ($save) {
            file_put_contents(getcwd() . "/app/Listeners/{$listenerName}.php", $code);
            $io->success("Listener saved to app/Listeners/{$listenerName}.php");
        }
