...
class MakeConsoleCommand extends Command
{
    ...
    {
        $this
            ->setName('laris:ai:make:command')
            ->setDescription('Generate a Laravel Artisan Command using OpenRouter AI');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $name = $io->ask('What is the name of the Artisan command class?', 'CleanupOldPostsCommand');

        $prompt = <<<PROMPT
{$defaultPrompt}

You are a Laravel 10 expert. Generate a Laravel Artisan command class named "{$name}" that deletes posts older than 30 days from the database. Use dependency injection, PSR-12, and Laravel Artisan conventions.
PROMPT;

        $io->section('🧠 Generating Artisan Command using AI...');
        sleep(3);
        ...
        $io->section('✅ AI Generated Command Code');
        $io->writeln("<info>$code</info>");

        $save = $io->confirm('Do you want to save this command to app/Console/Commands?', true);
        if ($save) {
            $path = getcwd() . "/app/Console/Commands/{$name}.php";
            file_put_contents($path, $code);
            $io->success("Command saved to app/Console/Commands/{$name}.php");
        }
